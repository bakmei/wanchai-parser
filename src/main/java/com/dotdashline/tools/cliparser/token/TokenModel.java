...
/**
 * This class is the root level of the model which associates the metadata with the incoming tokens.
 * 
 * Depends on the input tokens, this model may only associate with a subset of the
 * metadata.  e.g. The metadata has n defined options, however, the tokens only contains
 * n-1 of them.  Therefore, only n-1 {@link OptionToken} will be created and only n-1 {@link OptionMeta}
 * will be associated.
 *
...
public class TokenModel {
    ...
    TokenModel(MetaModel metaModel, String[] tokens) throws CLIParserException {
        ...
        if (tokens == null || tokens.length == 0) {
            /**
             * trigger the default command as the empty string will match with the default command.
             * To define a default command, {@see DefaultCommand}
             */
    ...
    /**
     * Associates the ParamMeta and transfer the rest of the elements from the queue into a list.
     *
     * @param tokens
     */
    private void createParamTokens(CommandToken commandToken, Queue<String> tokens) {
        ...
        // If the param type is an array, assign the rest of the tokens to the field.
        paramMeta.stream().forEach(x -> {
           if (x.isArray()) {
